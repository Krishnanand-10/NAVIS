...
(function () {
        ...
        if (window.__PRELOADED_NAVIS_DATA__) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    console.warn('[NAVIS] Sample endpoint failed, running fallback simulation.');
                    runSimulation();
                }
            } catch (err) {
                console.warn('[NAVIS] Server offline or fetch error, running simulation request.');
                runSimulation();
            }
        }
    }
    ...
    async function runSimulation() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                console.error('[NAVIS] Backend error:', errData);
    ...
    function render3DTrajectory(data) {
        ...
        const traces = [
            ...
            // 5: Current Position Vehicle Head Marker (EKF)
            {
                ...
                y: [tr.ai_ekf.y[0]],
                z: [tr.ai_ekf.z[0]],
                marker: { color: '#00f0ff', size: 8, symbol: 'diamond' },
                showlegend: false,
            }
        ];
    ...
    function animationLoop(timestamp) {
        ...
        const dt = AppState.data.metadata.dt || 0.02;
        const stepsToAdvance = (delta * AppState.playbackSpeed) / dt;
